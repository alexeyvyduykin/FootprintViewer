...
public class SwathCore22
{
    ...
    public SwathCore22(IList<Geo2D> near, IList<Geo2D> far, Func<double, bool> isCoverPolis) : this()
    {
    ...
    public SwathCore22(IList<(double lonRad, double latRad)> near, IList<(double lonRad, double latRad)> far, Func<double, bool> isCoverPolis) : this()
    {
        var NearPoints = new List<List<Geo2D>>();
        var FarPoints = new List<List<Geo2D>>();
        ...
        double curLon = 0.0;
        double curLat = 0.0;
        double oldNearLon = near[0].lonRad;
        double oldNearLat = near[0].latRad;

        for (int i = 0; i < near.Count; i++)
        {
            (curLon, curLat) = near[i];

            if (Math.Abs(curLon - oldNearLon) >= 3.2)
            {
                double cutLat = LinearInterpDiscontLat(new Geo2D(oldNearLon, oldNearLat), new Geo2D(curLon, curLat));
                ...
                if (oldNearLon > 0.0)
                {
                    ...
                    truePointsNear.Add(new Geo2D(curLon, curLat));
                }
                else
                {
                    truePointsNear.Add(new Geo2D(-Math.PI, cutLat));
        ...
        double oldFarLon = far[0].lonRad;
        double oldFarLat = far[0].latRad;
       ...
       // old = far[0];
        for (int i = 0; i < far.Count; i++)
        {
            ...
            if (Math.Abs(curLon - oldFarLon) >= 3.2)
            {
                double cutLat = LinearInterpDiscontLat(new Geo2D(oldFarLon, oldFarLat), new Geo2D(curLon, curLat));
                ...
                if (oldFarLon > 0.0)
                {
                    ...
                    truePointsFar.Add(new Geo2D(curLon, curLat));
                }
                else
                {
                    truePointsFar.Add(new Geo2D(-Math.PI, cutLat));
