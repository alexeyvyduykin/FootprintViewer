...
internal static class FootprintBuilder22
{
    private static readonly Random _random = new();
    private static readonly double _size = 1.2;
    private static readonly double _r = Math.Sqrt(_size * _size / 2.0);
    ...
    public static IList<Footprint> Create(Satellite satellite, IList<GroundTarget> targets)
    {
        var footprints = new List<Footprint>();
        int footprintIndex = 0;

        var sat = satellite.ToPRDCTSatellite();
        var sensor = satellite.ToPRDCTSensor();

        var swath1 = new Swath(sat.Orbit, satellite.LookAngleDeg, satellite.RadarAngleDeg, SwathMode.Left);
        var swath2 = new Swath(sat.Orbit, satellite.LookAngleDeg, satellite.RadarAngleDeg, SwathMode.Right);

        var swaths = new[] { swath1, swath2 };
        ...
        var arr = targets
            .Select(s => (s.Name, s.Points!.Centroid.Coordinate))
            .Select(s => (s.Coordinate.X, s.Coordinate.Y, s.Name!))
            .ToList();

        var res = new AvailabilityBuilder().Build(sat, sensor.MinLookAngleDeg, sensor.MaxLookAngleDeg, arr);
        ...
        foreach (var item in res)
        {
            ...
            var node = item.Node;
            var u = item.NadirU;
            var sensorIndex = (item.IsLeftSwath == true) ? SwathDirection.Left : SwathDirection.Right;

            var (t, center, border) = GetRandomFootprint(sat, swaths[(int)sensorIndex], node - 1, u);
   ...
   public static List<TaskAvailability> CreateObservationTaskAvailabilities33(IList<GroundTarget> groundTargets, IList<Satellite> satellites, IList<ITask> tasks)
    {
        var observationTasks = tasks.Where(s => s is ObservationTask).Cast<ObservationTask>().ToList();

        var list = new List<TaskAvailability>();

        var gts = groundTargets
            .Select(s => (s.Name, s.Points!.Centroid.Coordinate))
            .Select(s => (s.Coordinate.X, s.Coordinate.Y, s.Name!))
            .ToList();
        ...
        {
            var sensor = item.ToPRDCTSensor();
            var sat = item.ToPRDCTSatellite();
            var epoch = item.Epoch;
            var res = new AvailabilityBuilder().Build(sat, sensor.MinLookAngleDeg, sensor.MaxLookAngleDeg, gts);

            foreach (var task in observationTasks)
            {
                var dd = res.Where(s => string.Equals(s.Name, task.GroundTargetName)).ToList();
                var windows3 = new List<Interval>();

                foreach (var item4 in dd)
                {
                    var begin = item4.BeginTime * item4.Node;
                    var duration = item4.EndTime - item4.BeginTime;
                    var gtName = item.Name;
                    var interval = new Interval()
                    {
                        Begin = epoch.AddSeconds(begin),
                        Duration = duration
                    };
                    windows3.Add(interval);
                }
                var res3 = new TaskAvailability()
                {
                    TaskName = task.Name,
